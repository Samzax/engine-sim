...
foreach(_device keyboard mouse)
    ...
    if (_match EQUAL -1)
        message(FATAL_ERROR "Delta ${_device} aggregator changed; review cmake/DeltaInputFix.cmake")
    endif()
    string(REPLACE "${_old_poll}" "result = ${_poll} || result;" _input_code "${_input_code}")
    string(REPLACE "\"../include/" "\"${_delta_dir}/include/" _input_code "${_input_code}")
    set(_patched_source "${PROJECT_BINARY_DIR}/dependency-fixes/yds_${_device}_aggregator.cpp")
    set(_previous_code "")
    if (EXISTS "${_patched_source}")
        file(READ "${_patched_source}" _previous_code)
    endif()
